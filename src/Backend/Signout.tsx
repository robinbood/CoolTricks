...
const SignOut = async (req : Request) => {
    ...
    const newCookie = "sessionId= ; HttpOnly; sameSite=strict ; Path=/ , MaxAge=0"

    try {
        const exists =  await store.exists(sessionId)
        const [user] = await store.hmget(sessionId,[
            "userId"
        ])
        if (!user) {
            throw new Error("User not found")
        }
        const userID = await db.select().from(users).where(eq(users.id, parseInt(user)))
